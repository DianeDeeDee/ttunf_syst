...
//	
	m_hSvc.create1D("NSmallJets_chan_FatJet1", "; Number of small jets -1 Fat Jets ; Events", 20, 0, 20);
	m_hSvc.create1D("SmaljetsCloseLep_chan_FatJet1", "; Number of small jets close to lept-1 Fat Jets ; Events", 20, 0, 20);
	m_hSvc.create1D("SmaljetsFarLep_chan_FatJet1", "; Number of small jets far from lept-1 Fat Jets ; Events", 20, 0, 20);
	
	m_hSvc.create1D("NSmallJets_chan_FatJet2", "; Number of small jets -2 Fat Jets ; Events", 20, 0, 20);
	m_hSvc.create1D("SmaljetsCloseLep_chan_FatJet2", "; Number of small jets close to lept-2 Fat Jets ; Events", 20, 0, 20);
  ...
  for (int k = 0; k < e.largeJet().size(); k++) {
    ...
    if (e.largeJet()[k].passLoose()) {
      ...
      if (passdR && passdPhi && passM  && passEta && passPt &&passD12) {
        ...
        fatjets++;
        idx = k;
//std::cout<<"..................D12_1= "<<e.largeJet()[k].split12()<<std::endl;
      }
//std::cout<<"..................D12_2= "<<e.largeJet()[k].split12()<<std::endl;
    }
//std::cout<<"...................D12_3= "<<e.largeJet()[k].split12()<<std::endl;
...
if(fatjets>0)  h->h1D("largeJetM", "", s)->Fill((Jet_Fat[0]).M()*1e-3, weight);
  h->h1D("lepPt", "", s)->Fill(l.Perp()*1e-3, weight);
  h->h1D("jetPt", "", s)->Fill(j.Perp()*1e-3, weight);
  h->h1D("seljetPt", "", s)->Fill(selj.Perp()*1e-3, weight);
if(fatjets>0)  h->h1D("largeJetPt", "", s)->Fill(Jet_Fat[0].Perp()*1e-3, weight);
*/
  double mindr2 = 9999;
...
//std::cout <<"PlotSemi2: jetsmall= "<<jetsmall<<"  ;fatjets= "<<fatjets<<std::endl;
   ...
   if(m_electron){
//h->h1D("lepPt", "", s)->Fill(e.electron()[0].mom().Perp()*1e-3, weight);
  //      h->h1D("Eta", "", s)->Fill(e.electron()[0].mom().Eta(), weight);
       //h->h1D("ttbarMass", "", s)->Fill((Jet_Fat[0]+e.jet()[sel_jet].mom()+e.electron()[0].mom()+momNu).M()*1e-3, weight);
       //h->h1D("Ptttbar", "", s)->Fill((Jet_Fat[0]+e.jet()[sel_jet].mom()+e.electron()[0].mom()+momNu).Perp()*1e-3, weight);
...
//h->h1D("lepPt", "", s)->Fill(e.muon()[0].mom().Perp()*1e-3, weight);
//    h->h1D("Eta", "", s)->Fill(e.muon()[0].mom().Eta(), weight);
    //h->h1D("ttbarMass", "", s)->Fill((Jet_Fat[0]+e.jet()[sel_jet].mom()+e.muon()[0].mom()+momNu).M()*1e-3, weight);
    //h->h1D("Ptttbar", "", s)->Fill((Jet_Fat[0]+e.jet()[sel_jet].mom()+e.muon()[0].mom()+momNu).Perp()*1e-3, weight);     
