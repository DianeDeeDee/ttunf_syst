 ...
 if (jetsmall < 3 && fatjets < 1) return;
  ...
  vector<TLorentzVector> Jet_Fat;
   ...
   for (int k = 0; k < e.jet().size(); ++k) {
  	jetsmall++;
  	double tdr = e.jet()[k].mom().DeltaR(l);
  	double tdr2 = e.jet()[k].mom().DeltaR(Jet_Fat[0]);
    if(tdr > dr){
    	SjetsFarL = k;
    	if (SjetsFarL == sel_jet) continue;
        SmaljetsFarLep++;
        h->h1D("SjetsFarLepDR", "", s)->Fill(e.jet()[k].mom().DeltaR(Jet_Fat[0]), weight);
     }
   if(tdr > dr && tdr2 > 1.5){
   	    SjetsFarLepLJ = k; 
  ...
  int idxW = -1;
  int idxH = -1;
bool idxGstar_bool = 0;
/*  std::cout<<"1: idxGstar= "<<idxGstar<<" idxT= "<<idxT<<" idxW= "<<idxW<<"idxTop1= "<<idxTop1<<" idxZ= "<<idxZ<<" idxH= "<<idxH<<std::endl;
if( idxGstar == -1){  
idxGstar_bool = 1;
 for (int z = 0; z < e.partMom().size(); ++z) {
	if (std::fabs(e.partMom()[z].id()) == 999999) {
      idxGstar = z;
	  break;
	}
	//true_Gstar = e.partMom()[idxGstar].mom();
  }
  const TLorentzVector &true_Gstar = e.partMom()[idxGstar].mom();
 //Z = 23, W = 24, H = 25, t = 6, b = 5, G* = 999999, T = 999002 
if( idxGstar >= 0){
 h->h1D("MGStar_true", "", s)->Fill(true_Gstar.M()*1e-3,weight);
}
if(true_Gstar.M() == 0) std::cout<<"idxGstar= "<<idxGstar<<"  ; true_Gstar.M (in GeV)= "<<true_Gstar.M()*1e-3<<std::endl;
else std::cout<<"idxGstar= "<<idxGstar<<"  ; true_Gstar.M (in GeV)= "<<true_Gstar.M()*1e-3<<std::endl;
  for (int z = 0; z < e.partMom().size(); ++z) {
	if (std::fabs(e.partMom()[z].id()) == 999002) {
      idxT = z;
	  break;
	}
	//true_T = e.partMom()[idxT].mom();
  }
  const TLorentzVector &true_T = e.partMom()[idxT].mom();
  
  
//  for (int z = 0; z < e.partMom().size(); ++z) {
//	if (std::fabs(e.partMom()[z].id()) == 6) {
//      idxTop = z;
//	  break;
//	}
//	//true_Top = e.partMom()[idxTop].mom();
//  }
  
for (int k = 0; k < e.partMom().size(); ++k) {
  if (std::fabs(e.partMom()[k].id()) == 6) {
    if (idxTop1 == -1) idxTop1 = k;
    else idxTop2 = k;
  }
}

  const TLorentzVector &true_Top1 = e.partMom()[idxTop1].mom();
  const TLorentzVector &true_Top2 = e.partMom()[idxTop2].mom();
 
  for (int z = 0; z < e.partMom().size(); ++z) {
	if (std::fabs(e.partMom()[z].id()) == 23) {
      idxZ = z;
	  break;
	}
	//true_Z = e.partMom()[idxZ].mom();
  }
  const TLorentzVector &true_Z = e.partMom()[idxZ].mom();
 
  for (int z = 0; z < e.partMom().size(); ++z) {
	if (std::fabs(e.partMom()[z].id()) == 24) {
      idxW = z;
	  break;
	}
	//true_W = e.partMom()[idxW].mom();
  }
  const TLorentzVector &true_W = e.partMom()[idxW].mom();
  
  for (int z = 0; z < e.partMom().size(); ++z) {
	if (std::fabs(e.partMom()[z].id()) == 25) {
      idxH = z;
	  break;
	}
	//true_H = e.partMom()[idxH].mom();
  }
  const TLorentzVector &true_H = e.partMom()[idxH].mom();

 std::cout<<"2: idxGstar= "<<idxGstar<<" idxT= "<<idxT<<" idxW= "<<idxW<<"idxTop1= "<<idxTop1<<" idxZ= "<<idxZ<<" idxH= "<<idxH<<std::endl;
if( idxT >= 0) h->h1D("MT_true", "", s)->Fill(true_T.M()*1e-3,weight);
}*/ 
   if(m_electron){
//    	 Mej = (e.electron()[0].mom()+e.jet()[sel_jet].mom()).M();
    	 ...
    	 //Menu = (e.electron()[0].mom()+e.momNu).M();
    	    ...
    	    if(fatjets == 0 && SmaljetsFarLep == 2){
    	    	 ...
    	    	 /*    if(idxGstar_bool == 1){
 	 		        //New truth
    	    	     if ( idxGstar >= 0) {
			TLorentzVector reco_Gstar = (SjetsFarLep[0]+SjetsFarLep[1]+e.jet()[sel_jet].mom()+e.muon()[0].mom()+momNu);
			// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
			h->h1D("PtGStar_true_reco_2Sjets", "", s)->Fill((true_Gstar.Perp() - reco_Gstar.Perp())*1e-3, weight);
			h->h1D("MGStar_true_reco_2Sjets", "", s)->Fill((true_Gstar.M()- reco_Gstar.M())*1e-3, weight);
    	   			 }
    	   			 
    	   			 if (idxT >= 0) {
						TLorentzVector reco_T = (SjetsFarLep[0]+SjetsFarLep[1]);
						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
						h->h1D("PtT_true_reco_2Sjets", "", s)->Fill((true_T.Perp() - reco_T.Perp())*1e-3, weight);
						h->h1D("MT_true_reco_2Sjets", "", s)->Fill((true_T.M() - reco_T.M())*1e-3, weight);
    	   			 }
    	   			  if (idxTop1 >= 0) {
						TLorentzVector reco_Top = (e.jet()[sel_jet].mom()+e.muon()[0].mom()+momNu);
						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
						h->h1D("PtTop_true_reco_2Sjets", "", s)->Fill((true_Top1.Perp() - reco_Top.Perp())*1e-3, weight);
						h->h1D("MTop_true_reco_2Sjets", "", s)->Fill((true_Top1.M() - reco_Top.M())*1e-3, weight);
    	   			 }
    	   			 if (idxW >= 0) {
						TLorentzVector reco_W = (e.muon()[0].mom()+momNu);
						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
						h->h1D("PtW_true_reco_2Sjets", "", s)->Fill((true_W.Perp() - reco_W.Perp())*1e-3, weight);
						h->h1D("MW_true_reco_2Sjets", "", s)->Fill((true_W.M() - reco_W.M())*1e-3, weight);
    	   			 }
    	    	  ...
    	    	  if(fatjets == 1 && SmaljetsFarLepLJet==2){
    	    	     ...
    	    	     //New truth
//    	    	     if (idxGstar >= 0) {
//						TLorentzVector reco_Gstar = (Jet_Fat[0]+SjetsFarLepLJet[0]+SjetsFarLepLJet[1]+e.jet()[sel_jet].mom()+e.muon()[0].mom()+momNu);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtGStar_true_reco_1FatJet_2Sjets", "", s)->Fill((true_Gstar.Perp() - reco_Gstar.Perp())*1e-3, weight);
//						h->h1D("MGStar_true_reco_1FatJet_2Sjets", "", s)->Fill((true_Gstar.M()- reco_Gstar.M())*1e-3, weight);
//    	   			 }
//    	   			 
//    	   			 if (idxT >= 0) {
//						TLorentzVector reco_T = (Jet_Fat[0]+SjetsFarLepLJet[0]+SjetsFarLepLJet[1]);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtT_true_reco_1FatJet_2Sjets", "", s)->Fill((true_T.Perp() - reco_T.Perp())*1e-3, weight);
//						h->h1D("MT_true_reco_1FatJet_2Sjets", "", s)->Fill((true_T.M() - reco_T.M())*1e-3, weight);
//    	   			 }
//    	   			 
//    	   			 if (idxW >= 0) {
//						TLorentzVector reco_W = (e.muon()[0].mom()+momNu);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtW_true_reco_1FatJet_2Sjets", "", s)->Fill((true_W.Perp() - reco_W.Perp())*1e-3, weight);
//						h->h1D("MW_true_reco_1FatJet_2Sjets", "", s)->Fill((true_W.M() - reco_W.M())*1e-3, weight);
//    	   			 }
//    	   		     if (idxZ >= 0) {
//						TLorentzVector reco_Z = (SjetsFarLepLJet[0]+SjetsFarLepLJet[1]);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtZ_true_reco_1FatJet_2Sjets", "", s)->Fill((true_Z.Perp() - reco_Z.Perp())*1e-3, weight);
//						h->h1D("MZ_true_reco_1FatJet_2Sjets", "", s)->Fill((true_Z.M() - reco_Z.M())*1e-3, weight);
//    	   			 }
//    	   			 
//    	   			   if (idxH >= 0) {
//						TLorentzVector reco_H = (SjetsFarLepLJet[0]+SjetsFarLepLJet[1]);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtH_true_reco_1FatJet_2Sjets", "", s)->Fill((true_H.Perp() - reco_H.Perp())*1e-3, weight);
//						h->h1D("MH_true_reco_1FatJet_2Sjets", "", s)->Fill((true_H.M() - reco_H.M())*1e-3, weight);
//    	   			 }
    	   			 //End New truth	    	   	     
                 }
...
//    	    	     //New truth
//    	    	     if (idxGstar >= 0) {
//						TLorentzVector reco_Gstar = (Jet_Fat[0]+SjetsCloseLep[0]+SjetsCloseLep[1]+e.jet()[sel_jet].mom()+e.muon()[0].mom()+momNu);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtGStar_true_reco_1FatJet_2Sjets", "", s)->Fill((true_Gstar.Perp() - reco_Gstar.Perp())*1e-3, weight);
//						h->h1D("MGStar_true_reco_1FatJet_2Sjets", "", s)->Fill((true_Gstar.M()- reco_Gstar.M())*1e-3, weight);
//    	   			 }
//    	   			 
//    	   			 if (idxT >= 0) {
//						TLorentzVector reco_T = (Jet_Fat[0]+SjetsCloseLep[0]+SjetsCloseLep[1]);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtT_true_reco_1FatJet_2Sjets", "", s)->Fill((true_T.Perp() - reco_T.Perp())*1e-3, weight);
//						h->h1D("MT_true_reco_1FatJet_2Sjets", "", s)->Fill((true_T.M() - reco_T.M())*1e-3, weight);
//    	   			 }
...
//    	   			 }
//    	   		     if (idxZ >= 0) {
//						TLorentzVector reco_Z = (SjetsCloseLep[0]+SjetsCloseLep[1]);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtZ_true_reco_1FatJet_2Sjets", "", s)->Fill((true_Z.Perp() - reco_Z.Perp())*1e-3, weight);
//						h->h1D("MZ_true_reco_1FatJet_2Sjets", "", s)->Fill((true_Z.M() - reco_Z.M())*1e-3, weight);
//    	   			 }
//    	   			 
//    	   			   if (idxH >= 0) {
//						TLorentzVector reco_H = (SjetsCloseLep[0]+SjetsCloseLep[1]);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtH_true_reco_1FatJet_2Sjets", "", s)->Fill((true_H.Perp() - reco_H.Perp())*1e-3, weight);
//						h->h1D("MH_true_reco_1FatJet_2Sjets", "", s)->Fill((true_H.M() - reco_H.M())*1e-3, weight);
//    	   			 }
    	 	...
    	 	if (SmaljetsFarLep > 2 && fatjets == 0){
    	 		if (SmaljetsFarLep == 3){
    	 		     ...
    	 		     h->h1D("MassZprime3jets_3Jets", "", s)->Fill((SjetsFarLep[0]+SjetsFarLep[1]+SjetsFarLep[2]).M()*1e-3, weight);
    	 		       ...
    	 		       //New truth
//    	    	     if (idxGstar >= 0) {
//						TLorentzVector reco_Gstar = (SjetsFarLep[0]+SjetsFarLep[1]+SjetsFarLep[2]+e.jet()[sel_jet].mom()+e.muon()[0].mom()+momNu);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtGStar_true_reco_3Sjets", "", s)->Fill((true_Gstar.Perp() - reco_Gstar.Perp())*1e-3, weight);
//						h->h1D("MGStar_true_reco_3Sjets", "", s)->Fill((true_Gstar.M()- reco_Gstar.M())*1e-3, weight);
//    	   			 }
//    	   			 
//    	   			 if (idxT >= 0) {
//						TLorentzVector reco_T = (SjetsFarLep[0]+SjetsFarLep[1]+SjetsFarLep[2]);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtT_true_reco_3Sjets", "", s)->Fill((true_T.Perp() - reco_T.Perp())*1e-3, weight);
//						h->h1D("MT_true_reco_3Sjets", "", s)->Fill((true_T.M() - reco_T.M())*1e-3, weight);
//    	   			 }
//    	   			  if (idxTop1 >= 0) {
//						TLorentzVector reco_Top= (e.jet()[sel_jet].mom()+e.muon()[0].mom()+momNu);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtTop_true_reco_3Sjets", "", s)->Fill((true_Top1.Perp() - reco_Top.Perp())*1e-3, weight);
//						h->h1D("MTop_true_reco_3Sjets", "", s)->Fill((true_Top1.M() - reco_Top.M())*1e-3, weight);
//    	   			 }
//    	   			 if (idxW >= 0) {
//						TLorentzVector reco_W = (e.muon()[0].mom()+momNu);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtW_true_reco_3Sjets", "", s)->Fill((true_W.Perp() - reco_W.Perp())*1e-3, weight);
//						h->h1D("MW_true_reco_3Sjets", "", s)->Fill((true_W.M() - reco_W.M())*1e-3, weight);
//    	   			 }
    	 		 ...
    	 		 if (SmaljetsFarLep == 4){
                     ...
                     h->h1D("PtTop_had_4jets", "", s)->Fill((SjetsFarLep[0]+SjetsFarLep[1]+SjetsFarLep[2]).Perp()*1e-3, weight);  
                         	 		        ...
                         	 		        //New truth
//    	    	     if (idxGstar >= 0) {
//						TLorentzVector reco_Gstar = (SjetsFarLep[0]+SjetsFarLep[1]+SjetsFarLep[2]+SjetsFarLep[3]+e.jet()[sel_jet].mom()+e.muon()[0].mom()+momNu);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtGStar_true_reco_4Sjets", "", s)->Fill((true_Gstar.Perp() - reco_Gstar.Perp())*1e-3, weight);
//						h->h1D("MGStar_true_reco_4Sjets", "", s)->Fill((true_Gstar.M()- reco_Gstar.M())*1e-3, weight);
//    	   			 }
//    	   			 
//    	   			 if (idxT >= 0) {
//						TLorentzVector reco_T = (SjetsFarLep[0]+SjetsFarLep[1]+SjetsFarLep[2]+SjetsFarLep[3]);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtT_true_reco_4Sjets", "", s)->Fill((true_T.Perp() - reco_T.Perp())*1e-3, weight);
//						h->h1D("MT_true_reco_4Sjets", "", s)->Fill((true_T.M() - reco_T.M())*1e-3, weight);
//    	   			 }
//    	   			  if (idxTop1 >= 0) {
//						TLorentzVector reco_Top= (e.jet()[sel_jet].mom()+e.muon()[0].mom()+momNu);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtTop_true_reco_4Sjets", "", s)->Fill((true_Top1.Perp() - reco_Top.Perp())*1e-3, weight);
//						h->h1D("MTop_true_reco_4Sjets", "", s)->Fill((true_Top1.M() - reco_Top.M())*1e-3, weight);
//    	   			 }
//    	   			 if (idxW >= 0) {
//						TLorentzVector reco_W = (e.muon()[0].mom()+momNu);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtW_true_reco_4Sjets", "", s)->Fill((true_W.Perp() - reco_W.Perp())*1e-3, weight);
//						h->h1D("MW_true_reco_4Sjets", "", s)->Fill((true_W.M() - reco_W.M())*1e-3, weight);
//    	   			 }
//    	   		     if (idxZ >= 0) {
//						TLorentzVector reco_Z = (SjetsFarLep[0]+SjetsFarLep[1]);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtZ_true_reco_4Sjets", "", s)->Fill((true_Z.Perp() - reco_Z.Perp())*1e-3, weight);
//						h->h1D("MZ_true_reco_4Sjets", "", s)->Fill((true_Z.M() - reco_Z.M())*1e-3, weight);
//    	   			 }
//    	   			 
//    	   			 if (idxH >= 0) {
//						TLorentzVector reco_H = (SjetsFarLep[0]+SjetsFarLep[1]);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtH_true_reco_4Sjets", "", s)->Fill((true_H.Perp() - reco_H.Perp())*1e-3, weight);
//						h->h1D("MH_true_reco_4Sjets", "", s)->Fill((true_H.M() - reco_H.M())*1e-3, weight);
//    	   			 }
    	   			 //End New truth	
    	 		 ...
    	 		 if (SmaljetsFarLep > 4) {
    	 		    ...
    	 		    h->h1D("PtTop_had_5jets", "", s)->Fill((SjetsFarLep[0]+SjetsFarLep[1]+SjetsFarLep[2]).Perp()*1e-3, weight);
    	 		       	 		        //New truth
//    	    	     if (idxGstar >= 0) {
//						TLorentzVector reco_Gstar = (SjetsFarLep[0]+SjetsFarLep[1]+SjetsFarLep[2]+SjetsFarLep[3]+SjetsFarLep[4]+e.jet()[sel_jet].mom()+e.muon()[0].mom()+momNu);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtGStar_true_reco_5Sjets", "", s)->Fill((true_Gstar.Perp() - reco_Gstar.Perp()), weight);
//						h->h1D("MGStar_true_reco_5Sjets", "", s)->Fill((true_Gstar.M()- reco_Gstar.M())*1e-3, weight);
//    	   			 }
//    	   			 
//    	   			 if (idxT >= 0) {
//						TLorentzVector reco_T = (SjetsFarLep[0]+SjetsFarLep[1]+SjetsFarLep[2]+SjetsFarLep[3]+SjetsFarLep[4]);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtT_true_reco_5Sjets", "", s)->Fill((true_T.Perp() - reco_T.Perp())*1e-3, weight);
//						h->h1D("MT_true_reco_5Sjets", "", s)->Fill((true_T.M() - reco_T.M())*1e-3, weight);
//    	   			 }
//    	   			  if (idxTop1 >= 0) {
//						TLorentzVector reco_Top= (e.jet()[sel_jet].mom()+e.muon()[0].mom()+momNu);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtTop_true_reco_5Sjets", "", s)->Fill((true_Top1.Perp() - reco_Top.Perp())*1e-3, weight);
//						h->h1D("MTop_true_reco_5Sjets", "", s)->Fill((true_Top1.M() - reco_Top.M())*1e-3, weight);
//    	   			 }
//    	   			 if (idxW >= 0) {
//						TLorentzVector reco_W = (e.muon()[0].mom()+momNu);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtW_true_reco_5Sjets", "", s)->Fill((true_W.Perp() - reco_W.Perp())*1e-3, weight);
//						h->h1D("MW_true_reco_5Sjets", "", s)->Fill((true_W.M() - reco_W.M())*1e-3, weight);
//    	   			 }
//    	   		     if (idxZ >= 0) {
//						TLorentzVector reco_Z = (SjetsFarLep[0]+SjetsFarLep[1]);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtZ_true_reco_5Sjets", "", s)->Fill((true_Z.Perp() - reco_Z.Perp())*1e-3, weight);
//						h->h1D("MZ_true_reco_5Sjets", "", s)->Fill((true_Z.M() - reco_Z.M())*1e-3, weight);
//    	   			 }
//    	   			 
//    	   			 if (idxH >= 0) {
//						TLorentzVector reco_H = (SjetsFarLep[0]+SjetsFarLep[1]);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtH_true_reco_5Sjets", "", s)->Fill((true_H.Perp() - reco_H.Perp())*1e-3, weight);
//						h->h1D("MH_true_reco_5Sjets", "", s)->Fill((true_H.M() - reco_H.M())*1e-3, weight);
//    	   			 }
    	   			 //End New truth	
                         }
    	 	}
    	    if(fatjets > 1) {
                ...
                h->h1D("MassGStar2FatJet_2FatJets", "", s)->Fill((Jet_Fat[0]+Jet_Fat[1]).M()*1e-3, weight); 
                 ...
                 //New truth
//    	    	     if (idxGstar >= 0) {
//						TLorentzVector reco_Gstar = (Jet_Fat[0]+Jet_Fat[1]+e.jet()[sel_jet].mom()+e.muon()[0].mom()+momNu);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtGStar_true_reco_2FatJet", "", s)->Fill((true_Gstar.Perp() - reco_Gstar.Perp())*1e-3, weight);
//						h->h1D("MGStar_true_reco_2FatJet", "", s)->Fill((true_Gstar.M()- reco_Gstar.M())*1e-3, weight);
//    	   			 }
//    	   			 
//    	   			 if (idxT >= 0) {
//						TLorentzVector reco_T = (Jet_Fat[0]+Jet_Fat[1]);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtT_true_reco_2FatJet", "", s)->Fill((true_T.Perp() - reco_T.Perp())*1e-3, weight);
//						h->h1D("MT_true_reco_2FatJet", "", s)->Fill((true_T.M() - reco_T.M())*1e-3, weight);
//    	   			 }
//    	   			  if (idxTop1 >= 0) {
//						TLorentzVector reco_Top= (Jet_Fat[1]);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtTop_true_reco_2FatJet", "", s)->Fill((true_Top1.Perp() - reco_Top.Perp())*1e-3, weight);
//						h->h1D("MTop_true_reco_2FatJet", "", s)->Fill((true_Top1.M() - reco_Top.M())*1e-3, weight);
//    	   			 }
//    	   			 if (idxW >= 0) {
//						TLorentzVector reco_W = (e.muon()[0].mom()+momNu);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtW_true_reco_2FatJet", "", s)->Fill((true_W.Perp() - reco_W.Perp())*1e-3, weight);
//						h->h1D("MW_true_reco_2FatJet", "", s)->Fill((true_W.M() - reco_W.M())*1e-3, weight);
//    	   			 }
//    	   		     if (idxZ >= 0) {
//						TLorentzVector reco_Z = (Jet_Fat[0]);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtZ_true_reco_2FatJet", "", s)->Fill((true_Z.Perp() - reco_Z.Perp())*1e-3, weight);
//						h->h1D("MZ_true_reco_2FatJet", "", s)->Fill((true_Z.M() - reco_Z.M())*1e-3, weight);
//    	   			 }
//    	   			 
//    	   			 if (idxH >= 0) {
//						TLorentzVector reco_H = (Jet_Fat[0]);
//						// REMEMBER TO DECLARE MYHISTOGRAM AND CHANGE NAME
//						h->h1D("PtH_true_reco_2FatJet", "", s)->Fill((true_H.Perp() - reco_H.Perp())*1e-3, weight);
//						h->h1D("MH_true_reco_2FatJet", "", s)->Fill((true_H.M() - reco_H.M())*1e-3, weight);
//    	   			 }
    	   			 //End New truth	    	  
