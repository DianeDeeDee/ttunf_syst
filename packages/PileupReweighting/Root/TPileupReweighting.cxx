...
Root::TPileupReweighting::TPileupReweighting(const char* name) :
  TNamed(name,"notitle"), 
   ...
   m_nextPeriodNumber(1),m_ignoreFilePeriods(false),m_metadatatree(0),m_unrepDataTolerance(0.05)
{
...
Int_t Root::TPileupReweighting::Initialize() {
   ...
   if(toleranceViolated) {
      ...
      throw std::runtime_error("Throwing exception 222");
   }
...
Double_t Root::TPileupReweighting::GetDataWeight(Int_t runNumber, TString trigger, Double_t x) {
   ...
   if(m_countingMode) {
      ...
      throw std::runtime_error("You cannot get a weight when in config file generating mode. Please use FillWeight method");
   }
   ...
   if(numerHist==0) {
      ...
      throw std::runtime_error("Could not find unprescaled trigger. Please AddLumiCalc with a 'None' trigger");
   }
