...
double Analysis::AnalysisMuonConfigurableScaleFactors::scaleFactor(int charge,
								   ...
								   const std::pair<int,int>& bin) const {
...
void Analysis::AnalysisMuonConfigurableScaleFactors::read_file(
							   ...
							   bool &FileWithEffi, bool & m_finelybinned
							   ) const 
{
  ...
  // OPEN DATA FILE //
  ////////////////////
  string filecontent = ReadZippedEffiFile(TString(name));
  stringstream infile ;
  infile << filecontent;
  
//   istream infile(name.c_str());
//   if (infile.fail()) {
...
//   }
		  ...
		  if(parameter=="FileWithEffi")
		    {
		      ...
		      if(v>0.5)
			{
			  cout<<"Input File has absolute efficiency information"<<endl;
			  FileWithEffi=true;
			}
		    }
  ...
  if(conf==PerPeriod || conf==AverageOverPeriods)
      ...
      while (!(infile.fail() ||infile.eof())) 
	{
      ...
      if (!m_finelybinned && fabs(r_eta_min)>90 && fabs(r_phi_min)>90 && fabs(r_region) > 50){
        cout << " AnalysisMuonConfigurableScaleFactors: Detected that we are using finely binned scale factors! "<<endl;
        m_finelybinned= true;
      }
	  if (infile.fail() || infile.eof())
	    break;
...
//    }
  ...
  if(index<0) 
    ...
    for (unsigned int k=0; k<m_pt_min[index].size(); k++) 
    {
        ...
        if(m_UseChargeDependentSF && charge!=m_charge[index][k]) continue;
        
        // these are the fine eta phi bins
        if(m_is_finely_binned){
//             if(fineBin==0) continue; 
            if(fineBin==m_region[index][k]){
...
double Analysis::AnalysisMuonConfigurableScaleFactors::scaleFactorReplica(int charge,
									  const TLorentzVector & tlv, int nr,
									  const std::pair<int,int>& bin) const {
...
std::string Analysis::AnalysisMuonConfigurableScaleFactors::ReadZippedEffiFile(TString fname) const{
    
    gzFile file;
    file = gzopen (fname.Data(), "r");
    if (! file) {
        file = gzopen((fname+".gz").Data(),"r");
        if (!file){
            cerr << "AnalysisMuonConfigurableScaleFactors: Unable to read input file. This is *not* good. Bailing out."<<endl;
            return "";
        }
        else {
            cout << "AnalysisMuonConfigurableScaleFactors: You may be using an outdated input file name!\n"
                     << "Current recommendations can be found at:\n https://twiki.cern.ch/twiki/bin/viewauth/AtlasProtected/MCPAnalysisGuidelinesData2012\n"
                     << "All up to date files should have a .txt.gz filetype. "<<endl;
        }
    }
    string outs = "";
    ...
    while (1){
        ...
        int bytes_read = gzread(file,buffer, bufsize-1);
        buffer[bytes_read] = '\0';
        if (bytes_read != bufsize-1 ){
            if (gzeof(file)){
                outs+=string((const char*) buffer);
                break;
            }
            int err = 0;
            string err_str = gzerror (file, &err);
            if ( err){
                cerr << "AnalysisMuonConfigurableScaleFactors:  problem reading the compressed input: "<<endl;
                cerr << "\t"<<err_str<<endl;
                break;
            }
        }
