...
#include "ttResoSingleLepton/TTBarLeptonJetsBuilder_dRmin.h"

//==========================================================================================================
// Class constructor
//
TTBarLeptonJetsBuilder_dRmin::TTBarLeptonJetsBuilder_dRmin(TString units, bool verbose) {
  ...
  if(units == "MeV") {
    ...
    cout  << " ERROR in in TTBarLeptonJetsBuilder_dRmin :: Unit setting '" << units << "' doest not match pattern of GeV or MeV" << endl;
  }
}
...
//
void TTBarLeptonJetsBuilder_dRmin::Init(double highJetMass, double minJetPt, double dRCutOffset, double dRCutMassFactor) {
  //Set parameters
  m_highM           = highJetMass;
  m_minJetPt        = minJetPt;
...
//
bool TTBarLeptonJetsBuilder_dRmin::SetupEvent(vector<TLorentzVector*> &jets, TLorentzVector &lepton, TLorentzVector &neutrino) {

  if(!m_init) {
    cout << " ERROR in TTBarLeptonJetsBuilder_dRmin :: Tool not initialized! Initialize tool before usage!!!" << endl;
    return false;
  }
...
//
double TTBarLeptonJetsBuilder_dRmin::GetMttdRmin() {
...
//
TLorentzVector TTBarLeptonJetsBuilder_dRmin::GetTlvdRmin() {

  TLorentzVector tt_dRmin(0.0,0.0,0.0,0.0);
...
//
bool TTBarLeptonJetsBuilder_dRmin::FindJetIndices_HM() {
  ...
  if(closest_jetIdx_had >= 0){
    ...
    if(m_verbose) cout << " WARNING in TTBarLeptonJetsBuilder_dRmin :: Can not find jet close to high mass jet" << endl;
  }

  if(closest_jetIdx_lep >= 0) {
    m_jet_indices.push_back(closest_jetIdx_lep);
  } else {
    if(m_verbose) cout << " WARNING in TTBarLeptonJetsBuilder_dRmin :: Can not find jet close to leptonic W-boson" << endl;
  }
...
//
bool TTBarLeptonJetsBuilder_dRmin::FindJetIndices_LM() {
