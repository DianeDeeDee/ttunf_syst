...
void mergeResolvedSpectra()
{
  ...
  string outputFileName="outputs_may5_rush/lepPreSpectra_EWS.root";
  ...
  string nominalFileName="inputs_april22/MttSpectra_20140422.root";
  ...
  //channels.push_back("masstT_all_e");
  ...
  //channels.push_back("masstT_all_mu");
  ...
  vector<string> Bsystematics;
  Bsystematics.push_back("Btag_r1");
  Bsystematics.push_back("Btag_r2");
  Bsystematics.push_back("Btag_r3");
  Bsystematics.push_back("Btag_b1");
  Bsystematics.push_back("Btag_b2");
  Bsystematics.push_back("Btag_b3");

  Bsystematics.push_back("BtagC_r1");
  Bsystematics.push_back("BtagC_r2");
  Bsystematics.push_back("BtagC_r3");
  Bsystematics.push_back("BtagC_b1");
  Bsystematics.push_back("BtagC_b2");
  Bsystematics.push_back("BtagC_b3");

  Bsystematics.push_back("BtagL_r1");
  Bsystematics.push_back("BtagL_r2");
  Bsystematics.push_back("BtagL_r3");
  Bsystematics.push_back("BtagL_b1");
  Bsystematics.push_back("BtagL_b2");
  Bsystematics.push_back("BtagL_b3");
  ...
  */
  ...
  //sysSpectra=TFile::Open( "inputs/f_out_BtagSyst.root");

  //AddSystematicSet(NominalSpectra, sysSpectra, outfile, prefixes, systematics);
  //cout<<"AddSystematicSetBtag \n";
  //AddSystematicSetBtag(NominalSpectra, sysSpectra, outfile, prefixes, Bsystematics);
...
void Copy(TFile * input, TFile * outfile ,vector<string> prefixes){
  for(unsigned int p=0; p<prefixes.size(); p++){
    ...
    outfile->cd();
    Nominal->Write((prefixes.at(p)).c_str());
    //TH1D * Nominal= GetSpectra(outfile,("binkkg_"+prefixes.at(p)+"_allHad").c_str());
...
void AddSystematicSet(TFile * nominalFile, TFile *spectraFile, TFile * outfile, vector<string> prefixes, vector<string> systematics)
{
  ...
  for(unsigned int p=0; p<prefixes.size(); p++){
    ...
    for (unsigned int s=0; s<systematics.size(); s++){
       ...
       if(!up && ! dw) {
	 cout<<"Warning, up and dw histograms do not exist, using nominal \n";
	 
	 up=(TH1D*) NominalShift->Clone((prefixes.at(p)+"_"+systematics.at(s)+"_up").c_str());
         dw=(TH1D*) NominalShift->Clone((prefixes.at(p)+"_"+systematics.at(s)+"_dw").c_str());
	 
	 up->SetTitle((prefixes.at(p)+"_"+systematics.at(s)+"_up").c_str());
	 up->SetName((prefixes.at(p)+"_"+systematics.at(s)+"_up").c_str());
	 dw->SetTitle((prefixes.at(p)+"_"+systematics.at(s)+"_dw").c_str());
	 dw->SetName((prefixes.at(p)+"_"+systematics.at(s)+"_dw").c_str());
       }
       ...
       for(int b=0; b<=up->GetNbinsX()+1; b++)
	 {
       ...
       outfile->cd();
       up->Write( (prefixes.at(p)+"_"+systematics.at(s)+"_up").c_str());
       dw->Write( ( prefixes.at(p)+"_"+systematics.at(s)+"_dw").c_str());
       
       
       //check that histogram wrote properly
       up= GetSpectra(outfile, prefixes.at(p)+"_"+systematics.at(s)+"_up");
       dw= GetSpectra(outfile, prefixes.at(p)+"_"+systematics.at(s)+"_dw");

       cout<<"check that histogram wrote properly:(addSystematics) "<<upInt<<" "<<up->Integral()<<" "<<upInt-up->Integral()<<endl;
    ...
    cout<<"debug: deleting nominal histos \n";
    delete Nominal;
    delete scaleFactors;
  ...
  for(unsigned int p=0; p<prefixes.size(); p++){
    cout<<prefixes.at(p)<<endl;
	 ...
	 if(!up && !dw){
	   ...
	   up=(TH1D*) NominalShift->Clone((prefixes.at(p)+"_"+theSysBase+"_up").c_str());
	   dw=(TH1D*) NominalShift->Clone((prefixes.at(p)+"_"+theSysBase+"_dw").c_str());
	 }
       }
       else{
	 up=(TH1D*) NominalShift->Clone((prefixes.at(p)+"_"+theSysBase+"_up").c_str());
	 dw=(TH1D*) NominalShift->Clone((prefixes.at(p)+"_"+theSysBase+"_dw").c_str());
       ...
       up->SetTitle((prefixes.at(p)+"_"+theSys+"_up").c_str());
       up->SetName((prefixes.at(p)+"_"+theSys+"_up").c_str());
       dw->SetTitle((prefixes.at(p)+"_"+theSys+"_dw").c_str());
       dw->SetName((prefixes.at(p)+"_"+theSys+"_dw").c_str());
       ...
       for(int b=0; b<=up->GetNbinsX()+1; b++)
	 {
       ...
       outfile->cd();
       up->Write((prefixes.at(p)+"_"+theSys+"_up").c_str());
       dw->Write((prefixes.at(p)+"_"+theSys+"_dw").c_str());
  ...
  for(unsigned int p=0; p<prefixes.size(); p++){
    ...
    for (unsigned int s=0; s<systematics.size(); s++){
       ...
       for(int b=0; b<=up->GetNbinsX()+1; b++)
	 {
	   ...
	   //this will implicitly flip the sign of the smaller variation if bot are in the same direction
    ...
    TH1D * upSum=(TH1D*) Nominal->Clone((prefixes.at(p)+"_"+outSysName+"_up").c_str());
    TH1D * dwSum=(TH1D*) Nominal->Clone((prefixes.at(p)+"_"+outSysName+"_dw").c_str());
    
    upSum->SetTitle((prefixes.at(p)+"_"+outSysName+"_up").c_str());
    upSum->SetName((prefixes.at(p)+"_"+outSysName+"_up").c_str());

    dwSum->SetTitle((prefixes.at(p)+"_"+outSysName+"_dw").c_str());
    dwSum->SetName((prefixes.at(p)+"_"+outSysName+"_dw").c_str());
    ...
    outfile->cd();
    upSum->Write( (prefixes.at(p)+"_"+outSysName+"_up").c_str());
    dwSum->Write( ( prefixes.at(p)+"_"+outSysName+"_dw").c_str());
    
    cout<<"getting histograms \n";
    //check that histogram wrote properly
    upSum= GetSpectra(outfile, prefixes.at(p)+"_"+outSysName+"_up");
    dwSum= GetSpectra(outfile, prefixes.at(p)+"_"+outSysName+"_dw");
