...
void setupSampleHist(std::vector<std::string> &samples, std::vector<std::string> &paths, std::vector<std::string> &histograms, std::vector<std::string> &regionsList, std::string rebinningFile="")
{

  // Set samples and paths (X is a placeholder for flavors and MASS is a placeholder for the mass, e.g. 125)
  ...
  samples.push_back(std::string("wzzz")); paths.push_back(std::string("bkg/X/diboson/NonWW/ZW/ZWp + bkg/X/diboson/NonWW/ZW/ZWm + bkg/X/diboson/NonWW/ZZ"));
...
//  samples.push_back(std::string("wwew")); paths.push_back(std::string("bkg/X/diboson/WW/ - bkg/X/diboson/WW/ggWW - bkg/X/diboson/WW/qqWW"));
//  samples.push_back(std::string("wzzzew")); paths.push_back(std::string("bkg/X/diboson/NonWW/"));
  ...
  //samples.push_back(std::string("wjets")); paths.push_back(std::string("bkg/X/Wjets/bkg + bkg/X/Wjets/data"));
  samples.push_back(std::string("qcd")); paths.push_back(std::string("bkg/X/QCD"));
  ...
  if (rebinningFile!="") { //if a rebinning file is provided then we can read the list of histograms to dump from there
    ...
    std::cout << "Warning! Using hard-coded list of hists in caf2statHists.C"<<std::endl;
		  ...
		  std::string statDir = "./")
{
  ...
  std::vector<std::string> regionsList;
  std::string rebinningFileName="";
  if (useRebinningFile) rebinningFileName = "rev/" + version + "/rebinningList.txt";
  setupSampleHist(samples, paths, histograms, regionsList, rebinningFileName);
  ...
  //Check whether we need to use the ztautau NFs
  bool useSF_Ztautau_0jet=true, useSF_Ztautau_1jet=true;
  for (unsigned int iRegion = 0; iRegion<regionsList.size(); iRegion++)
    if (regionsList[iRegion]=="ztautaucr_0j")
      useSF_Ztautau_0jet=false;
    else if (regionsList[iRegion]=="ztautaucr_1j")
      useSF_Ztautau_1jet=false;
  ...
  if (syst=="_BJetWeightUp")
    ...
    syst = "MtagDown";
  else if (syst.find("_BJetWeight_") != std::string::npos) {
    syst = "Btag" + syst.substr(14) + syst.substr(12,1);
  }
  ...
  for (unsigned int iSample = 0; iSample < samples.size(); iSample++)      
  {
    ...
    double sf_Ztautau_0jet, sf_Ztautau_1jet;
    while (!ztautau_norms_file.eof())
    {
      ...
      else if (term == "em_signalLike_1j") sf_Ztautau_1jet = val;

    }
    ztautau_norms_file.close();
    std::cout << "Will apply following NFs:" << std::endl;
    if (useSF_Ztautau_0jet) std::cout << "  sf_Ztautau_0jet = " << sf_Ztautau_0jet << std::endl;
    if (useSF_Ztautau_1jet) std::cout << "  sf_Ztautau_1jet = " << sf_Ztautau_1jet << std::endl;
	...
	if (useSF_Ztautau_0jet) samplesFolder->setScaleFactor(name, sf_Ztautau_0jet, "bkg/?/Zjets/Nom/?/tt"); // Ztt 0j
      }
      else
      {
	if (useSF_Ztautau_1jet) samplesFolder->setScaleFactor(name, sf_Ztautau_1jet, "bkg/?/Zjets/Nom/?/tt"); // Ztt 1jet
      }
	...
	path = path.ReplaceAll("X", "ee") + " + " + TString(paths[iSample]).ReplaceAll("X", "mm");
      ...
      if (histograms[iHist].find("frec") != std::string::npos && histograms[iHist].find("Tag") == std::string::npos) {
...
//      std::cout << " Getting samplesFolder->getHistogram(" << path.Data() << ", " << compname.Data() << ")" << std::endl;
      ...
      if ((histograms[iHist].find("ASR") != std::string::npos || histograms[iHist].find("CZpeak") != std::string::npos) && histograms[iHist].find("Tag") == std::string::npos) {
