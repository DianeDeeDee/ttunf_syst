...
bool useSMTsyst          = 0;
bool useVGammaShapeLepPTsyst = 0;
...
void setup(double mass, bool alt = false, bool doOnlySignal=false)
{
  ...
  if (mode == 0 || mode == 1 || mode == 3 || mode == 4 || mode == 5 || mode == 6 || mode == 7 || mode == 8) // regions and cut specifications
  {
    ...
    Region sscr("sscr");
    sscr.isCR = true;
    if (splitSSCR) sscr.channels = channels;
    else           sscr.channels = crChannels;
    ...
    // WW SSCR for splitting sscr
    Region wwsscr("wwsscr");
    wwsscr.isCR = true;
    wwsscr.channels = crChannels;
...
//    if (do2DTopCut) tb.cuts.push_back(CutFunc("passedXtrTop" , &vars_ptr->passedXtrTop , "=="  , 1 , -1 , -1 ,  1 ));
	...
	if(splitSSCR) myRegions.push_back(wwsscr);
    }
  ...
  if (useDetSys)
  {
    ...
    if (useVGammaShapeLepPTsyst) {
      _fileNames.push_back(Sys("ATLAS_VGammaShapeLepPt"             , "VGammaShapeLepPtUp"  , "VGammaShapeLepPtDown" , 1 , 1 , "wg"));
      _fileNames.push_back(Sys("ATLAS_VGammaShapeLepPt"             , "VGammaShapeLepPtUp"  , "VGammaShapeLepPtDown" , 1 , 1 , "zjets"));
      _fileNames.push_back(Sys("ATLAS_VGammaShapeLepPt"             , "VGammaShapeLepPtUp"  , "VGammaShapeLepPtDown" , 1 , 1 , "zleplep"));
      _fileNames.push_back(Sys("ATLAS_VGammaShapeLepPt"             , "VGammaShapeLepPtUp"  , "VGammaShapeLepPtDown" , 1 , 1 , "ztautau"));
    }
    if (useOldBtag)
    {
...
bool skipRegion(Region* r, string channelName, string jetName)
{
  ...
  if (!(!doOF2jWWCR && merge2jWWCR && doWWCR2j && jetName == "2j" && r->name.find("mainControl") != string::npos && channelName != "ee" && channelName != "mm") && combineCRs && r->isCR && !r->isSFCR && channelName != "OF" && !r->isMCMSCR && !(r->name.find("sscr") != string::npos && splitSSCR) ) skipIt(skip,"inelegant 2jWWCR");  // inelegant fix for 2jWWCR
  ...
  if (combineCRs && !combineSFCRs && r->isCR && channelName != "OF" && channelName != "mm" && channelName != "ee" && !r->isMCMSCR && !(r->name.find("sscr") != string::npos && splitSSCR)  ) skipIt(skip,"");
