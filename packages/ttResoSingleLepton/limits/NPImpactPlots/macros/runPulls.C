	      ...
	      string loglevel = "DEBUG")
{
    ...
    bool isBinned = true;
    std::cout << "Activate binned likelihood calculation for binned models (true/false): " << isBinned << std::endl;
    if(isBinned){
      ...
      while((arg = iter.next())) {
        if (arg->IsA() == RooRealSumPdf::Class()) {
          ...
          //coutP(InputArguments) << "Measurement::initialise(" << modelConfigName << ") activating BinnedLikelihood for " << arg->GetName() << endl;
        }
      }
    }

    ModelConfig* mc = (ModelConfig*)ws->obj(modelConfigName);
    std::cout << "Set ModelConfig: " << modelConfigName << std::endl;
    if (!mc) {
        LOG(logERROR) << "ModelConfig: " << modelConfigName << " doesn't exist!";
        std::cout << "ModelConfig: " << modelConfigName << std::endl;
        exit(1);
    }

    RooDataSet* data = (RooDataSet*)ws->data(dataName);
    std::cout << "Set RooDataSet: " << dataName << std::endl;
    if (!data) {
    ...
    std::cout << "Set pois... parsed.size() = " << parsed.size() << std::endl;
    for (int i = 0; i < parsed.size(); i++) {
	std::cout << "parsed[i]= " << parsed[i] << std::endl;
        RooRealVar* poi = (RooRealVar*)ws->var(parsed[i].c_str());
        std::cout << "Name of POI " << poi->GetName() << std::endl;
    ...
    std::cout << "Get nuisance parameters from ModelConfig" << std::endl;
    if (!nuis) {
    ...
    std::cout << "Get global observables from ModelConfig" << std::endl;
    if (!globs) {
    ...
    while ((var = (RooRealVar*)itr->Next())) {
        ...
        if (varName.find("gamma_stat") != string::npos || (varName.find("PM_EFF_f_recoil") != string::npos && varName.find("alpha") == string::npos)) {
    ...
    for (int in = 0; in < nrNuis; in++) {
        ...
        if (nuipName.find("LTAGBREAK1") != string::npos) continue; // don't consider this NP
