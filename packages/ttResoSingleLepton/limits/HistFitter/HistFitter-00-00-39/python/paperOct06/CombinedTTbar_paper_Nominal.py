...
if not (noSyst or excludeSyst == "Norm_QCDe"):
    smallBgrSample.addSystematic(Systematic("Norm_QCDe","Nom","Norm_QCDeHigh","Norm_QCDeLow","tree","overallSys"))
if not (noSyst or excludeSyst == "Norm_QCDmu"):
    smallBgrSample.addSystematic(Systematic("Norm_QCDmu","Nom","Norm_QCDmuHigh","Norm_QCDmuLow","tree","overallSys"))
...
#print "A: addSystematics Pttt"
#if not (noSyst or excludeSyst == "Pttt"):
#    for myChannel in sRegs :
#        myChannel.getSample("tt").addSystematic(Systematic("Pttt","Nom","PtttHigh","PtttLow","tree","overallHistoSys"))

#print "A: addSystematics hdamp"
#if not (noSyst or excludeSyst == "hdamp"):
    #for myChannel in sRegs :
        #myChannel.getSample("tt").addSystematic(Systematic("hdamp","Nom","hdampHigh","hdampLow","tree","overallHistoSys"))
...
if not (noSyst or "EWS" in excludeSyst):
    ## tt only
    for myChannel in sRegs :
        myChannel.getSample("tt").addSystematic(Systematic("EWS","Nom","EWSHigh","EWSLow","tree","overallHistoSys"))
        

print "A: addSystematics JER"
...
if not (noSyst or excludeSyst == "PDF"):
    for mySampleName in (["tt", "W"] + sigSampleNameList) :
        for myChannel in sRegs :
...
for N in [1, 13]:
#for N in [1, 13, 14, 15, 16]:
#for N in range(1,17):
    jesN="BoostedJES"+str(N)
    print "A: addSystematics "+jesN
    if not (noSyst or jesN in excludeSyst):
        for mySampleName in (allBgr + sigSampleNameList) :
...
print "A: addSystematics BoostedJESothers"
if not (noSyst or "BoostedJESothers" in excludeSyst):
    for mySampleName in (allBgr + sigSampleNameList) :
        for myChannel in sRegsBoosted :
            myChannel.getSample(mySampleName).addSystematic(Systematic("BoostedJESothers","Nom","BoostedJESothersHigh","BoostedJESothersLow","tree","overallHistoSys"))
...
if JESTYPE == 1 :
    ...
    print "A: addSystematics JESsmall "
...
#if not (noSyst or excludeSyst == "Whfsf"):
  #for myChannel in sRegs :
    #myChannel.getSample("W").addSystematic(Systematic("Whfsf","Nom","WhfsfHigh","WhfsfLow","tree","overallHistoSys"))
