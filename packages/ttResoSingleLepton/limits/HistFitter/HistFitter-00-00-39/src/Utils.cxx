...
//     TCanvas* c_corr = new TCanvas(canName.Data(),canName.Data(),1200,800); // .c_str()) 
    TCanvas* c_corr = new TCanvas(canName.Data(),canName.Data(),2400,1600);
    ...
    gStyle->SetMarkerSize(2*orig_MarkerSize);
    gStyle->SetMarkerColor(orig_MarkerColor);
    gStyle->SetPaintTextFormat(orig_PaintTextFormat) ;
    gStyle->SetLabelSize(2*orig_LabelSize);
    gStyle->SetOptStat(00000000);
    
    h_corr->SetMarkerSize(2.5*h_corr->GetMarkerSize());
    h_corr->SetMarkerColor(kBlack);
    Logger << kINFO << "h_corr->GetMarkerSize() " << h_corr->GetMarkerSize() << GEndl;
    Logger << kINFO << "gStyle->GetMarkerSize() " << gStyle->GetMarkerSize() << GEndl;
    ...
    Logger << kINFO << "Writing matrix content to file." << GEndl;
    Logger << kINFO << "h_corr->GetNbinsX() " << h_corr->GetNbinsX() << GEndl;
    Logger << kINFO << "h_corr->GetNbinsY() " << h_corr->GetNbinsY() << GEndl;
    ofstream outNumbers;
    outNumbers.open("corrMatrixNumbers.txt");
    for( int i = 0; i < h_corr->GetNbinsX(); i++){
      for( int j = 0; j < h_corr->GetNbinsY(); j++){
      
	const char * xLabel  = h_corr->GetXaxis()->GetBinLabel(i);
	const char * yLabel  = h_corr->GetYaxis()->GetBinLabel(j);
	const char * isSystX = strstr(xLabel, "alpha_");
	const char * isSystY = strstr(yLabel, "alpha_");
	
	if( isSystX && isSystY ){
	  Logger << kINFO << xLabel << " " << yLabel << ": h_corr->GetBinContent(i,j) = " << h_corr->GetBinContent(i,j) << GEndl;
	  outNumbers << h_corr->GetBinContent(i,j) << "\t";
	}
      }
      outNumbers << std::endl;
    }
