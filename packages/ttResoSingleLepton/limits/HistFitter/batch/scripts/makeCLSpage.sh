...
for point in "${sigSamples[@]}"
  do
  echo '<tr>'
  echo '<td> '${point}' </td>'
  echo '</tr>'
  echo '<tr>'  
  echo '<td><img border="0" src="upperlimit_cls_poi_'${point}'_noClean_Asym_CLs_grid_ts3.root.png" height="300"/> </td>'
  echo '<td><img border="0" src="upperlimit_cls_poi_'${point}'_Asym_CLs_grid_ts3.root.png" height="300" /> </td>'
  echo '</tr>'
