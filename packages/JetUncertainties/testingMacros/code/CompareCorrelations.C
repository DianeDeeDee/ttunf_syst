...
const bool varyPt  = true;
const bool varyEta = true;
const bool varyEtaCentral = true;
const bool addMultijet = true;

const double minZpt = -0.1;
const double maxZpt = 0.1;
const double minZeta = -0.15;
const double maxZeta = 0.15;

typedef enum { ptE, etaE } Variable;
typedef MultijetJESUncertaintyProvider MJP;

TLatex *tex;


std::vector<TString> getSpecialNames()
{
    return JESUtils::Vectorize("Flavor_Comp,Flavor_Uncert,Weak2012_FComp1,Weak2012_FComp2,Weak2012_FUncert1,Weak2012_FUncert2",",");
}

std::vector<int> getSpecialIndices(const TString configFile)
{
    const bool weaker2012 = configFile.Contains("InsituJES2012")&&configFile.Contains("WeakerCorrelation") ? true : false;
...
std::vector<int> getNormalIndices(MJP* mjp)
{
    std::vector<int> toReturn;
    for (int iComp = 0; iComp < mjp->getNUncertaintyComponents(); ++iComp)
        if (mjp->getComponentName(iComp).Contains("Pileup"))
            continue;
        else
            toReturn.push_back(iComp);
    return toReturn;
}

std::vector<int> getIndices(MJP* mjp,const TString configFile="")
{
    std::vector<int> toReturn = getNormalIndices(mjp);
    std::vector<int> extra = getSpecialIndices(configFile);
    for (size_t iComp = 0; iComp < extra.size(); ++iComp)
        toReturn.push_back(extra.at(iComp));
    return toReturn;
}

TString getComponentName(MJP* mjp, const int index)
{
    if (index >= 0)
        return mjp->getComponentName(index);
    return getSpecialNames().at(-(index+1));
}

double getUncertainty(MJP* mjp, const int index, const double pt, const double eta)
{
    if (index >= 0)
        return mjp->getRelUncertComponent(index,pt,eta);
    else if (index == -1)
        return mjp->getRelFlavorCompUncert(pt,eta,true,0);
    else if (index == -2)
        return mjp->getRelFlavorResponseUncert(pt,eta,0);
    else if (index == -3)
        return mjp->getRelFlavorCompUncertWeak1(pt,eta,0);
    else if (index == -4)
        return mjp->getRelFlavorCompUncertWeak2(pt,eta,0);
    else if (index == -5)
        return mjp->getRelFlavorResponseUncertWeak1(pt,eta,0);
    else if (index == -6)
        return mjp->getRelFlavorResponseUncertWeak2(pt,eta,0);

    printf("Failed to get uncertainty component of index %d\n",index);
    return -1;
}

double getCov(MJP *mjp, const std::vector<int>& indices, const Variable var, const double vary1, const double vary2, const double fixed)
{
    double cov=0;
    
    for (unsigned int i = 0; i < indices.size(); ++i)
        if (var==ptE)
            cov += getUncertainty(mjp,indices.at(i),vary1,fixed)*getUncertainty(mjp,indices.at(i),vary2,fixed);
        else if (var==etaE)
            cov += getUncertainty(mjp,indices.at(i),fixed,vary1)*getUncertainty(mjp,indices.at(i),fixed,vary2);
    
    return cov;
}

double getCorr(MJP* mjp, const std::vector<int>& indices, const Variable var, const double vary1, const double vary2, const double fixed)
{
    return getCov(mjp,indices,var,vary1,vary2,fixed)/sqrt(getCov(mjp,indices,var,vary1,vary1,fixed)*getCov(mjp,indices,var,vary2,vary2,fixed));
}


TH2D* MakeCorrMatrix(MJP* mjp, const std::vector<int>& indices, const Variable var, const std::vector<double>& variableBins, const double fixed) {
  ...
  for (int xi=1;xi<=Nbins;++xi)
    for (int yi=1;yi<=Nbins;++yi) {
      ...
      //printf("(%2d,%2d), %s: (%6.1f,%6.1f), corr: %.5f\n",xi,yi,var==ptE ? "pT" : "eta",vary1,vary2,getCorr(mjp,indices,var,vary1,vary2,fixed));
      hcorr->SetBinContent(xi,yi,getCorr(mjp,indices,var,vary1,vary2,fixed));
    }
...
TH2D *Subtract(TH2D *h1, TH2D *h2) {
  ...
  diff->SetStats(0);
  //diff->GetZaxis()->SetRangeUser(minZ,maxZ);
  //diff->GetZaxis()->SetRangeUser(-0.30,0.30);
  //diff->SetMinimum(-0.30); diff->SetMaximum(0.30);
...
TH1D *GetNuis(MJP *mjp, const int index, Variable var, VecD variableBins, double fixed) {
  static int i=0;
  ...
  for (int i=1;i<=nuis->GetNbinsX();++i) {
    ...
    if (var==ptE)
      nuis->SetBinContent(i,getUncertainty(mjp,index,vary,fixed));
    else if (var==etaE)
      nuis->SetBinContent(i,getUncertainty(mjp,index,fixed,vary));
  }
  nuis->SetStats(0);
  TString axisTitle = (var==ptE)? "#it{p}_{T}^{jet} [GeV]" : "jet #eta";
...
TH1D *GetTotUncert(MJP *mjp, Variable var, VecD variableBins, double fixed) {
  static int i=0;
  ...
  for (int i=1;i<=tot->GetNbinsX();++i) {
    ...
    if (var==ptE)
      tot->SetBinContent(i,mjp->getRelUncert(vary,fixed,0,true,false,0,0));
    else if (var==etaE)
      tot->SetBinContent(i,mjp->getRelUncert(fixed,vary,0,true,false,0,0));
  }
  tot->SetStats(0);
  TString axisTitle = (var==ptE)? "#it{p}_{T}^{jet} [GeV]" : "jet #eta";
...
void *DrawNuis(MJP *mjp, const std::vector<int>& indices, Variable var, VecD variableBins, double fixed) {
  vector<TH1*> nuis; std::vector<int> myIndices;
  for (int i=0;i<indices.size();++i)
  {
      nuis.push_back(GetNuis(mjp,indices[i],var,variableBins,fixed));
      myIndices.push_back(indices[i]);
  }
  ...
  tex->SetTextAlign(11);
  for (int i=0;i<nuis.size();++i) { tex->SetTextColor(cols[i%8]); tex->DrawLatex(0.18,0.88-0.05*i,getComponentName(mjp,myIndices[i])); }
}
...
void FormatAndDrawNuisanceParamPlot(MJP* jes1,MJP* jes2,TString JESconfig2,TString jetDesc, const std::vector<int>& indices,Variable var,VecD variableBins,double fixed)
{
  ...
  if (var == ptE)
    ...
    gPad->SetLogx(false);
  gPad->SetLogy(false);
  DrawNuis(jes2,indices,var,variableBins,fixed);
...
void CreateFormatAndPlotMatrices(MJP* jes1,MJP* jes2,const std::vector<int>& indices1, const std::vector<int>& indices2,TString JESconfig1,TString JESconfig2,TString jetDesc,TString psName,Variable var,VecD varyBins,VecD fixedVals)
{
  ...
  for (int ifix=0; ifix<fixedVals.size(); ifix++)
  {
    ...
    TH2D* corr1 = MakeCorrMatrix(jes1,indices1,var,varyBins,fixed);
    corr1->GetZaxis()->SetRangeUser(-0.0001,1.0001);
    c->cd(1);
    ...
    TH2D* corr2 = MakeCorrMatrix(jes2,indices2,var,varyBins,fixed);
    corr2->GetZaxis()->SetRangeUser(-0.0001,1.0001);
    c->cd(2);
    ...
    diff->GetZaxis()->SetRangeUser(var==ptE?minZpt:minZeta,var==ptE?maxZpt:maxZeta);
    c->cd(3);
    ...
    {
      c->cd(4);
      FormatAndDrawNuisanceParamPlot(jes1,jes2,JESconfig2,jetDesc,indices2,var,varyBins,fixed);
    }
  ...
  TString pdf= TString(psName).ReplaceAll(".ps",".pdf");
  ...
  if (MAKE_CONF_PLOTS)
  {
    TString confpdf = TString(confpsName).ReplaceAll(".ps",".pdf");
...
int main(int argc, char **argv) {
  ...
  TString MultijetConfig="MultijetJES_2011.config";
  std::vector<TString> jetAlgos = JESUtils::Vectorize("AntiKt4TopoEM",",");
  ...
  if (argc>4) {
    ...
    JESconfig1=argv[2];
    JESconfig2=argv[3];
    MultijetConfig=argv[4];
  }
  if (argc>5) jetAlgos = JESUtils::Vectorize(argv[5],",");
  if (argc>6) MCtype=argv[6];
  ...
  for (int jeti = 0; jeti < jetAlgos.size(); ++jeti)
  {
    ...
    MJP *jes1, *jes2;
    jes1 = new MJP(MultijetConfig,JESconfig1,jetAlgos[jeti],MCtype,sharePath);
    jes2 = new MJP(MultijetConfig,JESconfig2,jetAlgos[jeti],MCtype,sharePath);
    ...
    std::vector<int> indices1 = getIndices(jes1,JESconfig1);
    std::vector<int> indices2 = getIndices(jes2,JESconfig2);
    //printf("%d/%d, %d/%d\n",indices1.size(),jes1->getNUncertaintyComponents(),indices2.size(),jes2->getNUncertaintyComponents());
    
    TString jetDesc(Form("Anti-#it{k}_{t} R = 0.%d, %s+JES",
          	   jetAlgos[jeti].Contains("Kt4")?4:6,
          	   jetAlgos[jeti].Contains("EM")?"EM":"LCW"));
    
    TString name= TString(gSystem->BaseName(JESconfig2)).ReplaceAll(".config",".ps");
    TString dir="Correlation_Plots";
    ...
    if (varyPt)
    {
      ...
      TString psName = dir+"/Corr_vs_pt_"+jetAlgos[jeti]+"_"+name;
      
      // Do all the work
      CreateFormatAndPlotMatrices(jes1,jes2,indices1,indices2,JESconfig1,JESconfig2,jetDesc,psName,var,varyPtBins,fixedEtaVals);
    ...
    if (varyEta)
    {
      var = etaE;
      // Create the output file name
      TString psName = dir+"/Corr_vs_eta_"+jetAlgos[jeti]+"_"+name;
      
      // Do all the work
      CreateFormatAndPlotMatrices(jes1,jes2,indices1,indices2,JESconfig1,JESconfig2,jetDesc,psName,var,varyEtaBins,fixedPtVals);
    }
    if (varyEtaCentral)
    {
      var = etaE;
      // Create the output file name
      TString psName = dir+"/Corr_vs_etaCentral_"+jetAlgos[jeti]+"_"+name;

      // Do all the work
      CreateFormatAndPlotMatrices(jes1,jes2,indices1,indices2,JESconfig1,JESconfig2,jetDesc,psName,var,varyEtaCentralBins,fixedPtCentralVals);
    }
