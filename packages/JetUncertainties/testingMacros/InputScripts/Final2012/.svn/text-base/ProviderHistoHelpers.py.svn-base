...
def ExtendPtRangeOfHisto(histo,histoName):
    ...
    if fabs(ptBins[-1] - 2500) < 1.e-4 and fabs(ptBins[0] - 15) < 1.e-4:
        print "Histo already was [15,2500] GeV, doing nothing"
        return histo
    elif ptBins[-1] > 2500:
        print "Histo beyond range, cannot extend"
        return None
    elif ptBins[0] < 15:
        print "Histo below range, cannot extend"
        return None
    if not fabs(ptBins[-1] - 2500) < 1.e-4:
        ptBins.append(2500.)
    if not fabs(ptBins[-1] - 15) < 1.e-4:
        ptBins.insert(0,15.)
    ...
    for xBin in range(1,newHisto.GetNbinsX()+1):
        pt = newHisto.GetXaxis().GetBinCenter(xBin)
        if pt <= histo.GetXaxis().GetBinLowEdge(1):
            pt = histo.GetXaxis().GetBinLowEdge(1) + 1.e-4
        elif pt >= histo.GetXaxis().GetBinLowEdge(histo.GetNbinsX()+1):
            pt = histo.GetXaxis().GetBinLowEdge(histo.GetNbinsX()+1) - 1.e-4

        for yBin in range(1,newHisto.GetNbinsY()+1):
            eta = newHisto.GetYaxis().GetBinCenter(yBin)
            newHisto.SetBinContent(xBin,yBin,histo.GetBinContent(histo.FindBin(pt,eta)))
