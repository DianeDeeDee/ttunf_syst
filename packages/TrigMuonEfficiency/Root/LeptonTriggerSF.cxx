...
LeptonTriggerSF::setStorage(Storage& storage) const
{
  ...
  ::Info("LeptonTriggerSF::setStorage",
         "Year = %d, nperiods = %zu", m_year, storage.period->size());
  ::Info("LeptonTriggerSF::setStorage",
         "nquality = %zu, nbins = %zu, ntype = %zu, nregion = %zu",
         storage.quality->size(), storage.bins->size(), storage.type->size(), storage.region->size());
