...
std::vector<TObjArray*> *Root::TElectronEfficiencyCorrectionTool::buildToyMCTable(TObjArray *sf, TObjArray* eig, TObjArray* stat, TObjArray* uncorr, std::vector<TObjArray*> *corr)
{
  ...
  std::cout << "DEBUG in " << this->getName() 
            ...
            << "m_Rndm->Uniform(0, 1) after throwing "<< m_randomCounter << " random numbers: " << m_Rndm->Uniform(0, 1) << std::endl;
}
  return tmpVec;
...
//Check if the first file can be opened (needed for auto-setting of the seed based on the md5-sum of the file)
  ...
  if (m_debug){ std::cout << "DEBUG in " << this->getName() 
                  << " (file: " << __FILE__ << ", line: " << __LINE__ << ")! " 
  "Found "<< nRunNumbersFast << " run number ranges for fast sim with a total of " <<m_fastHistList["sf"].size()<<" scale factor histograms."<<std::endl; }
      if (m_debug){ std::cout << "DEBUG in " << this->getName() 
                  << " (file: " << __FILE__ << ", line: " << __LINE__ << ")! " 
  << "Found "<< nRunNumbersFull << " run number ranges for full sim with a total of " <<m_histList["sf"].size()<<" scale factor histograms."<<std::endl; }
                                                                         ...
                                                                         )
{
  ...
  if (nSF > 1)
  {
    std::cout<<"More than 1 SF found for eta="<<yValue<<" , et = "<<et<<" , run number = "<<runnumber<<" . Please check your input files!"<<std::endl;
  }
...
int Root::TElectronEfficiencyCorrectionTool::getFirstToyMCPosition()
{
  ...
  if (m_nToyMC>0)
  ...
  else
    {
      std::cerr << "ERROR in " << this->getName() 
                << " (file: " << __FILE__ << ", line: " << __LINE__ << ") "
                << "! No toy scale factors booked, returning the position of the scale factor! This is most likely not what you want!" << std::endl;
            return 0;
    }
}
...
int Root::TElectronEfficiencyCorrectionTool::getFirstCorrSysPosition()
{
  ...
  if (m_position_corrSys.size()>0)
  ...
  else
    {
      std::cerr << "ERROR in " << this->getName() 
                << " (file: " << __FILE__ << ", line: " << __LINE__ << ") "
                << "! No correlated uncertainties found, returning the position of the scale factor! This is most likely not what you want!" << std::endl;
            return 0;
    }
}
...
void Root::TElectronEfficiencyCorrectionTool::printResultMap()
{
  if (!m_isInitialized){
    ...
    std::cout << "INFO in " << this->getName() 
              << " (file: " << __FILE__ << ", line: " << __LINE__ << ") "
              << "! Printing map of result with name \"" << m_resultName << "\""<<std::endl;
    std::cout<<std::endl;
    std::cout<<"############"<<std::endl;
    std::cout << "Position  \t -\t Description " << std::endl;

    for (unsigned int pos=0; pos<m_result.getNResults(); pos++)
      std::cout<<pos<<" \t \t - \t "<<m_result.getResultDescription(pos)<<std::endl;
    std::cout<<"############"<<std::endl;
    std::cout<<std::endl;
}
...
void Root::TElectronEfficiencyCorrectionTool::printDefaultReturnMessage(TString reason, int line)
{
    ...
    std::cerr << "ERROR in " << this->getName() 
              ...
              << reason+"\n"
              << "Returning scale factor 1.0 with uncertainty 1.0" << std::endl;
}
