...
m_hSvc.create1D("Eta1", "Eta(Lepton) at low dR", 30, -3., 3);
...
m_hSvc.create1D("seljetPt1", " Selected Jet p_{T} (GeV) at low dR; Selected Jet p_{T} (GeV)", 20, 0, 1000);
 m_hSvc.create1D("seljetPt", " Selected Jet p_{T} (GeV) ; Selected Jet p_{T} (GeV)", 20, 0, 1000); //old: 10, 0, 1400
  m_hSvc.create1D("jetPt", " Jet p_{T} (GeV) ; Jet p_{T} (GeV)", 20, 0, 1000); //30 0 3000
...
//	m_hSvc.create1D("Nfatjets0", "; Size of Fat jets ; Events", 20, 0, 20);
	...
	m_hSvc.create1D("SmaljetsCloseLep_chan_FatJet0", "; Number of small jets close to lept-0 Fat Jets ; Events", 20, 0, 20);
	m_hSvc.create1D("SmaljetsFarLep_chan_FatJet0", "; Number of small jets far from lept-0 Fat Jets ; Events", 20, 0, 20);
//	
	m_hSvc.create1D("NSmallJets_FatJet1", "; Number of small jets -1 Fat Jets ; Events", 20, 0, 20);
	m_hSvc.create1D("SmaljetsCloseLep_FatJet1", "; Number of small jets close to lept-1 Fat Jets ; Events", 20, 0, 20);
	m_hSvc.create1D("SmaljetsFarLep_chan_FatJet1", "; Number of small jets far from lept-1 Fat Jets ; Events", 20, 0, 20);
	
	m_hSvc.create1D("NSmallJets_FatJet2", "; Number of small jets -2 Fat Jets ; Events", 20, 0, 20);
	m_hSvc.create1D("SmaljetsCloseLep_FatJet2", "; Number of small jets close to lept-2 Fat Jets ; Events", 20, 0, 20);
	m_hSvc.create1D("SmaljetsFarLep_chan_FatJet2", "; Number of small jets far from lept-2 Fat Jets ; Events", 20, 0, 20);
	
	m_hSvc.create1D("MET_plus_mtW", "; MET+mtW ; Events", 50, 0, 100);
	m_hSvc.create1D("mtW", "; mtW ; Events", 50, 0, 100);

//0 Fat jet
   m_hSvc.create1D("MassGStar_3SjFarNSeljL", "; GStar Mass (3Sjets + W) (GeV)", 36, 0, 3600);
  m_hSvc.create1D("PtGStar_3SjFarNSeljL", "; GStar p_{T} (3Sjets + W) (GeV) ; Events", 20, 0, 2000);
   ...
   m_hSvc.create1D("MassGStar_5SjFarNSeljL", "; GStar Mass (5Sjets + W) (GeV)", 36, 0, 3600);
...
//  m_hSvc.create1D("MassGStar_4SjFarNSeljL", "; GStar Mass (4Sjets + W) (GeV)" ,36, 0, 3600); 
  m_hSvc.create1D("MassGStar_4jets", "; GStar Mass (4Sjets + W) (GeV)" ,36, 0, 3600);
...
m_hSvc.create1D("MassZprime_2jets", "; Zprime_2jets Mass (GeV) ; Events", 20, 0, 2000);
  ...
  m_hSvc.create1D("MassGStar_1Fatj2SjClosNSeljL", "; GStar Mass (2Sjets + 1Fat jets + W) (GeV)", 36, 0, 3600);

 m_hSvc.create1D("Pt_1Fatj2SjClos", " 1FatJet_2jetsCloseToLep p_{T} (GeV) ; Events", 40, 0, 2000);
                 
  m_hSvc.create1D("PtGStar_1Fatj2SjClosNSeljL", "; GStar p_{T} (2Sjets + 1Fat jets + W) (GeV) ; Events", 40, 0, 4000);

 m_hSvc.create1D("Mass_1Fatj2SjClos", " M(Jet_Fat[0]+SjetsCloseLep[0]+SjetsCloseLep[1]) (GeV)", 360, 0, 3600);
 m_hSvc.create1D("MassZprimeFatJet_FatJet0", " ZPrime_1FatJet: M(FatJet[0]) (GeV)", 36, 0, 3600);
 m_hSvc.create1D( "Pt_1Fatj", " 2Sjets_1FatJet: FatJet[0] p_{T} (GeV); Events", 40, 0, 2000);//done old: 20, 0, 2000
 m_hSvc.create1D( "Pt_SjClos0", " 1FatJet_2jets: JetCloseLep[0] p_{T} (GeV); Events", 40, 0, 2000);//done old: 20, 0, 2000
 m_hSvc.create1D( "Pt_SjClos1", " 1FatJet_2jets: JetCloseLep[1] p_{T} (GeV); Events", 40, 0, 2000);//done old: 20, 0, 2000
 m_hSvc.create1D("Mass_1Fatj", " 1FatJet_2jets: M(FatJet[0]) (GeV)", 50, 0, 1000); //done old: 36, 0, 3600
 m_hSvc.create1D("Eta_1Fatj", " 1FatJet_2jets: Eta(FatJet[0])", 30, -3., 3); //done
 m_hSvc.create1D("Eta_SjClos0", " 1FatJet_2jets: Eta(JetCloseLep[0])", 30, -3., 3); //done?
 m_hSvc.create1D("Eta_SjClos1", " 1FatJet_2jets: Eta(JetCloseLep[1])", 30, -3., 3); //done?
 m_hSvc.create1D( "PtZprimeFatJet_W", " 1FatJet: W p_{T} (GeV); Events", 40, 0, 2000);  
 m_hSvc.create1D( "Pt_2SjClos", " 2jets_1FatJet: Jet[1+2] p_{T} (GeV); Events", 40, 0, 2000);
 
 m_hSvc.create1D("ttbarMass", "ttbar mass with a fat Jet (GeV)", 200, 0, 2000);
  m_hSvc.create1D("largeJetM", "Large jet Mass (GeV)", 100, 0, 1000);
  m_hSvc.create1D("Mtop", "Leptonic Top Mass (GeV)", 100, 0, 1000);
   m_hSvc.create1D("largeJetPt", "Fat jet Pt  (GeV)", 20, 0, 1000);
  m_hSvc.create1D("Pttop", "Leptonic Top Pt (GeV)", 20, 0, 1000);
 m_hSvc.create1D("Ptttbar","Pt ttbar with a Fat jet (GeV); Events", 20, 0, 1000);

//2 fat jets
 m_hSvc.create1D("Mass_2Fatj", " M(Jet_Fat[0]+Jet_Fat[1]) (GeV)", 360, 0, 3600);
 m_hSvc.create1D("Pt2FatJet_FatJet1", " FatJet_1 p_{T} (GeV) ; Events", 40, 0, 2000); //done
 ...
 m_hSvc.create1D("Pt2FatJet", " FatJets p_{T} (GeV) ; Events", 40, 0, 2000);
 m_hSvc.create1D("Mass2FatJet_FatJet0", " First Large jet M (GeV); Events", 40, 0, 800); //done old: 100, 0, 1000
 m_hSvc.create1D("PtGStar_2FatjNSeljL", " GStar p_{T} (2Fat jets + W) (GeV) ; Events", 40, 0, 2000);
 m_hSvc.create1D("Mass2FatJet_FatJet1", " Second Large jet M (GeV); Events", 80, 0, 800); //old: 100, 0, 1000
 m_hSvc.create1D("MassGStar_2FatjNSeljL", " GStar Mass (2Fat jets + W) (GeV)", 360, 0, 3600);
 m_hSvc.create1D("Pt2FatJet_FatJet0", " FatJet_0 p_{T} (GeV) ; Events", 40, 0, 2000); //done
 m_hSvc.create1D("dR_closejet", " dR (lepton,closestJet) ; dR (lepton,closestJet)", 15, 0., 3.); //done 
 m_hSvc.create1D("dR_seljet", " dR (lepton,SelJet) ; dR (lepton,SelJet)", 15, 0., 3.); //done
 m_hSvc.create1D("dR_bjet", " dR (lepton,bjet) ; dR (lepton,SelJet)", 15, 0., 3.); //done
}
 ...
 if (e.passReco()) {// events passes cuts from EventCutterReco
  ...
  for (int k = 0; k < e.largeJet().size(); k++) {
    ...
    if (e.largeJet()[k].passLoose()) {
      ...
      passPt = e.largeJet()[k].mom().Perp() > 300e3;//160e3;//true
      passdPhi = e.largeJet()[k].mom().DeltaPhi(l) > 2.3;//true
      passD12 = e.largeJet()[k].split12() > 40e3;//20e3;//40e3;//true
...
//      passD12 = e.largeJet()[k].split12() > 20e3;//40e3;//true
  ...
  h->h1D("D12", "", s)->Fill((e.largeJet()[k].split12())/1000., weight);
  }
  if(fatjets < 1 ) return;
  ...
  h->h1D("Eta", "", s)->Fill(l.Eta(), weight);  
   ...
   if(m_electron){
    ...
    if (fatjets == 1) {	
		...
		h->h1D("SmaljetsCloseLep_FatJet1", "", s)->Fill(SmaljetsCloseLep, weight);
	}
	if(fatjets > 0) {
		if(SmaljetsCloseLep > 1){
    		    h->h1D("Pt2jets", "", s)->Fill((SjetsCloseLep[0]+SjetsCloseLep[1]).Perp()*1e-3, weight);
				h->h1D("MassGStar_1Fatj2SjClosNSeljL", "", s)->Fill((Jet_Fat[0]+SjetsCloseLep[0]+SjetsCloseLep[1]+e.jet()[sel_jet].mom()+e.electron()[0].mom()+momNu).M()*1e-3, weight);
				h->h1D("PtGStar_1Fatj2SjClosNSeljL", "", s)->Fill((Jet_Fat[0]+SjetsCloseLep[0]+SjetsCloseLep[1]+e.jet()[sel_jet].mom()+e.electron()[0].mom()+momNu).Perp()*1e-3, weight);
				h->h1D("Pt_1Fatj2SjClos", "", s)->Fill((Jet_Fat[0]+SjetsCloseLep[0]+SjetsCloseLep[1]).Perp()*1e-3, weight);
				h->h1D("Pt_2SjClos", "", s)->Fill((SjetsCloseLep[0]+SjetsCloseLep[1]).Perp()*1e-3, weight);
				h->h1D("Mass_1Fatj2SjClos", "", s)->Fill((Jet_Fat[0]+SjetsCloseLep[0]+SjetsCloseLep[1]).M()*1e-3, weight); 
				...
				h->h1D("Pt_SjClos0", "", s)->Fill((SjetsCloseLep[0]).Perp()*1e-3, weight); 
				h->h1D("Pt_SjClos1", "", s)->Fill((SjetsCloseLep[1]).Perp()*1e-3, weight); 
				h->h1D("Eta_SjClos0", "", s)->Fill((SjetsCloseLep[0]).Eta(), weight);
				h->h1D("Eta_SjClos1", "", s)->Fill((SjetsCloseLep[1]).Eta(), weight);
	...
	if(fatjets > 0) {
		if(SmaljetsCloseLep > 1){
    		    h->h1D("Pt2jets", "", s)->Fill((SjetsCloseLep[0]+SjetsCloseLep[1]).Perp()*1e-3, weight);
				h->h1D("MassGStar_1Fatj2SjClosNSeljL", "", s)->Fill((Jet_Fat[0]+SjetsCloseLep[0]+SjetsCloseLep[1]+e.jet()[sel_jet].mom()+e.muon()[0].mom()+momNu).M()*1e-3, weight);
				h->h1D("PtGStar_1Fatj2SjClosNSeljL", "", s)->Fill((Jet_Fat[0]+SjetsCloseLep[0]+SjetsCloseLep[1]+e.jet()[sel_jet].mom()+e.muon()[0].mom()+momNu).Perp()*1e-3, weight);
				h->h1D("Pt_1Fatj2SjClos", "", s)->Fill((Jet_Fat[0]+SjetsCloseLep[0]+SjetsCloseLep[1]).Perp()*1e-3, weight);
				h->h1D("Pt_2SjClos", "", s)->Fill((SjetsCloseLep[0]+SjetsCloseLep[1]).Perp()*1e-3, weight);
				h->h1D("Mass_1Fatj2SjClos", "", s)->Fill((Jet_Fat[0]+SjetsCloseLep[0]+SjetsCloseLep[1]).M()*1e-3, weight); 
				h->h1D("Pt_1Fatj", "", s)->Fill((Jet_Fat[0]).Perp()*1e-3, weight);
