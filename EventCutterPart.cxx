...
EventCutterPart::EventCutterPart() {
}

EventCutterPart::~EventCutterPart() {
}

bool EventCutterPart::select(const Event &e, Event &sel) {
  int els = 0;
  int mus = 0;
  int fatjets = 0;
  int jets = 0;
  ...
  for (int k = 0; k < e.partLargeJet().size(); ++k) {
    if (e.partLargeJet()[k].pass()) {
      bool passdR = sel.partJet()[sel_jet].mom().DeltaR(e.partLargeJet()[k].mom()) > 1.5;
      bool passdPhi = e.partLargeJet()[k].mom().DeltaPhi(lepton) > 2.3;
      bool passM = e.partLargeJet()[k].mom().M() > 100e3;
